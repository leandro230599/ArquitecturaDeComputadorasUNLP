...
;       FUNCIONA PERO FALLA Y NO SE PORQUE
...
.code
lwu $s0, DIR_CONTROL($0)                        ; Cargo en $s0 CONTROL
lwu $s1, DIR_DATA($0)                           ; Cargo en $s1 DATA
jal ingreso                                     ; Salto a la subrutina ingreso
dadd $a0, $zero, $v0                            ; Guardo en $a0 el retorno de la subrutina ingreso $v0 que tiene 1 si es valido, 0 sino
dadd $a1, $zero, $v1                            ; Guardo en $a1 el retorno de la subrutina ingreso $v1 que tiene el digito
jal muestra                                     ; Salto a la subrutina muestra
halt
